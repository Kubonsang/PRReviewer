...
| `focus` | 이 리뷰어가 놓치면 안 되는 검토 카테고리 3–5개. 프로젝트 기술 스택에 맞게 작성 |
| `ignore` | 이 리뷰어의 관심 밖인 항목 2–4개. 다른 리뷰어가 담당하거나 범위 밖인 것 |
| `rules` | 명시적으로 체크할 구체적인 코드 규칙 목록 (선택). 아래 참조 |
| `output_language` | `"ko"` |
...
`rules` 는 diff에서 **반드시 위반 여부를 체크**해야 할 구체적인 규칙 목록이다.
`focus` 가 "무엇을 중점적으로 볼 것인가"의 카테고리라면, `rules` 는 "이 코드가 이 규칙을 지켰는가"의 체크리스트다.

각 항목은 문자열 또는 객체로 작성한다:

```json
"rules": [
  "async 함수는 반드시 try-catch로 감싼다",
  {
...
- 문자열: 판단 기준이 명확한 간단한 규칙
- 객체: `rule`(필수) + `reason`(위반 시 왜 문제인지) + `example`(올바른 코드 예시), 둘 다 선택

**`rules` 항목 작성 지침:**
- "확인한다", "있는지 본다" 가 아니라 위반 조건을 명확히 서술한다
  - 나쁨: "에러 처리를 확인한다"
  - 좋음: "async 함수에 try-catch가 없으면 위반"
- 프로젝트 env.json의 `conventions` 와 `key_dependencies` 를 참고해 관련 규칙을 제안한다
- 이 리뷰어의 `focus` 범위 내 규칙만 포함한다

JSON 구성 후, 사용자에게 커스텀 규칙 추가 여부를 묻는다:
```
이 리뷰어에 체크할 코드 규칙을 추가할까요?
예) "컴포넌트 props는 interface로 정의한다"
    "SQL 쿼리에 파라미터 바인딩을 사용한다"
추가할 규칙을 입력하거나, 없으면 엔터:
```
사용자가 규칙을 입력하면 `rules` 배열에 추가한다. 여러 개 입력 가능, 엔터만 누르면 건너뛴다.
